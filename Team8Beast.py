...
class Team8Beast(object):
    ...
    def __init__(self):
        ...
        self.lure=False #flag try to attract enemy beasts by hiding
        self.worldMapStateArray=None
        self.surrounding=''
        self.lastSurrounding=None
    ...
    def bewegen(self, energy, surrounding):
        ...
        if '?' in surrounding:
            if self.lastSurrounding:
                if '>' in self.lastSurrounding:
                    assertion='>'
                elif '<' in self.lastSurrounding:
                    assertion='<'
                else: 
                    assertion='>'
                self.surrounding.replace('?',assertion)
            else:
                surrounding.replace('?','<')
        ...
        destination= self. surroundingInspection('=')
        if destination!=None:
                self.lastSurrounding=surrounding
                return self.returnMethod(destination)
        ...
        #searches for smaller beasts in not reachable destinations and set a near destination to chase it
        if not self.chased:
            destination= self.preventiveInspection('<')     
            if destination!=None:
                    self.chased=True
                    return self.returnMethod(destination)
        
        #searches for smaller beasts in not reachable destinations hide if it was already chased one time
        if self.chased and not self.lure:
            destination= self.preventiveInspection('<')     
            if destination!=None:
                self.lure=True
                return self.returnMethod('?')
        
        #in next we are allowed to chase again
        self.lure=False
        self.chased=False
    ...
    def returnMethod(self,destination):
        self.lastSurrounding=self.surrounding
        return destination
